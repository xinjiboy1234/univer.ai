...
当现有公式不满足您需求的时候，可以通过以下多种方式来注册自定义公式。
...
2. 定义国际化
    ...
    定义你所需要的国际化内容，详细的字段说明请参考 [如何在 Formula Engine 中添加公式](/guides/sheet/customization/formula/#如何在-formula-engine-中添加公式) 的部分。同样的，多个公式就用公式名称作为 `key` 值区分。
...
1. 参考我们的 [贡献指南](/guides/sheet/contributing)， fork [Univer 项目](https://github.com/dream-num/univer)，从 dev 切分支开发。分支名称统一为 `feat/formula-[函数名称]`，比如 `feat/formula-sumif`。
...
正式着手写代码之前，推荐大家先学习 [公式的架构](/guides/sheet/architecture/formula)，对公式系统有一个全面的了解。
...
2. 国际化文件
    ...
    ![office excel](./formula/office-excel.png)

    函数描述和参数描述从 Office 函数详情页参考

    ![sumif](./formula/sumif.png)
...
- 任何公式的入参和出参都可以是 `A1`、`A1:B10`，单元格内容也可能是数字、字符串、布尔值、空单元格、错误值、数组等，虽然公式教程中说明了识别固定的数据类型，但是程序上实现是需要都兼容的，调研 Excel 的时候需要把所有情况考虑到，比如 `=SIN(A1:B10)`，会展开一个正弦计算后的范围。
  ...
  - 例如 `SUMIF` 函数，大家以为是求和，但是它是可以根据第二个参数进行展开的
        ...
        ![sumif array](./formula/sumif-array.png)

        ![sumif array result](./formula/sumif-array-result.png)

  - Excel 的公式计算，越来越像 numpy，比如

        ![numpy](./formula/numpy.png)
